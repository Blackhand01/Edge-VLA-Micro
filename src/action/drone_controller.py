...
from mavsdk.telemetry import Battery, FlightMode

from src.action.state import DroneHealthError, DroneState, DroneStateError, HealthReport
...
class DroneController:
    ...
    async def connect(self) -> None:
        ...
        await self._wait_until(lambda: self.state.armed is not None and self.state.health is not None, self.timeout_s, "telemetry state timeout")
    ...
    async def check_health(self, *, require_armable=True, require_global_position=True, require_battery=True) -> HealthReport:
        ...
        report = HealthReport(
            ...
            battery_remaining=normalise_battery_remaining(battery),
            battery_voltage_v=battery.voltage_v if battery is not None else None,
            flight_mode=self.state.flight_mode,
        )
        validate_health_report(report, require_armable, require_global_position, require_battery, self.battery_min_remaining)
        logger.info("Health OK: armable=%s gps=%s battery=%s armed=%s in_air=%s mode=%s", report.armable, report.gps_ok, "n/a" if report.battery_remaining is None else f"{report.battery_remaining:.0%}", report.armed, report.in_air, report.flight_mode)
        return report
    ...
    async def move_velocity(self, vx: float, vy: float, vz: float, yaw_deg: float) -> None:
        ...
        try:
            await self._ensure_offboard_started(yaw_deg)
            logger.info("Sending velocity setpoint vx=%.2f vy=%.2f vz=%.2f yaw=%.1f", vx, vy, vz, yaw_deg)
            self._offboard_setpoint = VelocityNedYaw(vx, vy, vz, yaw_deg)
    ...
    async def _ensure_offboard_started(self, yaw_deg: float) -> None:
        if self._offboard_active:
            return
        ...
        await self._wait_until(lambda: self.state.flight_mode == FlightMode.OFFBOARD, self.timeout_s, "offboard mode confirmation timeout")
...
def validate_health_report(report, require_armable, require_global_position, require_battery, battery_min_remaining) -> None:
    failures = []
    ...
    if require_battery and report.battery_remaining is None:
        failures.append("battery telemetry unavailable")
    elif require_battery and report.battery_remaining < battery_min_remaining:
        failures.append(f"battery below threshold: {report.battery_remaining:.0%}")
    if failures:
        raise DroneHealthError("; ".join(failures))


def normalise_battery_remaining(battery: Optional[Battery]) -> Optional[float]:
    if battery is None:
        return None
    return battery.remaining_percent / 100.0 if battery.remaining_percent > 1.0 else battery.remaining_percent
